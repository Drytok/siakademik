                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h4 class="modal-title" id="modalcreate">New Create Jadwal Pelajaran</h4>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                            aria-hidden="true">&times;</span></button>
                                </div>
                                <div class="modal-body">
                                    {{-- @dd($murid) --}}
                                    <form action="/jadwal_pelajaran" method="POST">
                                        {{ csrf_field() }}
                                        <div class="item form-group">
                                            <label class="col-form-label col-md-3 col-sm-3 label-align" for="last-name">Jam
                                                Masuk
                                                <span class="required">*</span>
                                            </label>
                                            <div class="col-md-6 col-sm-6 ">
                                                <input type="time" id="last-name" name="jam_masuk" required="required"
                                                    class="form-control">
                                            </div>
                                        </div>
                                        <div class="item form-group">
                    ...
                    <table class="table table-striped jambo_table bulk_action">
                        ...
                        <tbody>
                            @foreach ($jadwal_pelajaran as $item)
                                ...
                                @include('modaljadwalpelajaran.delete')
                                @include('modaljadwalpelajaran.edit')
                            @endforeach
                        </tbody>
                    </table>
                </div>
